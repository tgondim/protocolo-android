...
public class ListaViasActivity extends ListActivity implements OnItemClickListener {
	...
	private void atualizarVia() {
		ArrayList<Via> listaVias = RepositorioVias.getRepositorio(this).listar(ordenacao);
		ViaAdapter viaAdapter = new ViaAdapter(this, listaVias);
		setListAdapter(viaAdapter);
	} 
	
	@Override
	protected void onListItemClick (ListView l, View v, int position, long id) {
		Intent intent = new Intent("CADASTRO_VIAS");
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
		case 2:
			RepositorioVias.getRepositorio(this).excluir(info.id);
			atualizarVia();
			break;
